...
int compareType(Type* type1, Type* type2) {
  ...
  if (type1->typeClass != type2->typeClass){
    ...
    return 0;
  }else if(type1->arraySize != 0 &&type1->elementType->typeClass != type2->elementType->typeClass){
    return 0;
  }else{
    return 1;
  }
...
void freeType(Type* type) {
  // TODO - free elementType them type
  if (type->arraySize != 0){
...
/******************* Object utilities ******************************/
// NOTE: khởi tạo các variable của object utilities dựa trên thiết kế trong slide
...
Object* createFunctionObject(char *name) {
  ...
  functionObject->funcAttrs->paramList = (ObjectNode*)malloc(sizeof(ObjectNode));
  functionObject->funcAttrs->returnType = (Type*)malloc(sizeof(Type));
...
Object* createProcedureObject(char *name) {
  ...
  procedureObject->procAttrs->paramList = (ObjectNode*)malloc(sizeof(ObjectNode));
...
Object* createParameterObject(char *name, enum ParamKind kind, Object* owner) {
  ...
  parameterObject->paramAttrs->type = (Type*)malloc(sizeof(Type));
...
void freeObject(Object* obj) {
  // TODO
  // NOTE: thực hiện xóa các thuộc tính bên trong của object rồi sau đó mới free object để không có bộ nhớ chiếm dụng
  if (obj != NULL){
    switch (obj->kind){
      case OBJ_CONSTANT:
...
void freeScope(Scope* scope) {
  // TODO
  // NOTE: thực hiện xóa lần lượt các Object bên trong scope rồi sau đó mới free scope để không có bộ nhớ chiếm dụng
  if (scope != NULL){
...
void freeObjectList(ObjectNode *objList) {
  // TODO
  // NOTE: xóa lần lượt các objectList trong objList từ vị trí thứ 2 tới cuối bằng cách skip và cuối cùng là xóa vị trí đầu
  if (objList != NULL){
      ObjectNode* temp;
...
void freeReferenceList(ObjectNode *objList) {
  // TODO
  // NOTE: tương tự như free object list
  if (objList != NULL){
      ObjectNode* temp;
      while (objList->next != NULL){
        temp = objList->next;
